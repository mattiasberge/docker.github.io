...
| `--cpu-period=<value>` | Specify the CPU CFS scheduler period, which is used alongside —cpu-quota. There's one cpu-period per logical core. Defaults to 100 micro-seconds. Most users do not change this from the default. If you use Docker 1.13 or higher, use —cpus instead.

| `--cpu-quota=<value>`  | Impose a CPU CFS quota on the container. The number of microseconds per `--cpu-period` that the container is limited to before throttled, as such acting as the effective ceiling. For instance, give a container half of the period as quota and it will be limited to .5 CPU. If it uses two whole CPUs for one `--cpu-period` (4x its quota) it will be penalised over the next three `--cpu-periods`, by not being scheduled on any CPU at all.  If you use Docker 1.13 or higher, use `—cpus` instead.
